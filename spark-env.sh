...
TMP_PYSPARK_PYTHON=${PYSPARK_PYTHON:-'/opt/cloudera/parcels/Anaconda-4.0.0/bin/python'}
# If PYSPARK_DRIVER_PYTHON is unset, set to CDH_PYTHON
TMP_PYSPARK_DRIVER_PYTHON=${PYSPARK_DRIVER_PYTHON:-/opt/cloudera/parcels/Anaconda-4.0.0/bin/python}

if [ -n "$TMP_PYSPARK_PYTHON" ] && [ -n "$TMP_PYSPARK_DRIVER_PYTHON" ]; then
  export PYSPARK_PYTHON="$TMP_PYSPARK_PYTHON"
  export PYSPARK_DRIVER_PYTHON="$TMP_PYSPARK_PYTHON"
fi
